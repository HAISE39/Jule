...
export default function Sidebar() {
  ...
  useEffect(() => {
    const handleUpdate = () => {
      const updatedHistory = localStorage.getItem('vt_chat_history');
      if (updatedHistory) {
        try {
    ...
    // Listen for custom event to update history
    window.addEventListener('vt_history_updated', handleUpdate);
    return () => window.removeEventListener('vt_history_updated', handleUpdate);
  }, []);
      ...
      >
        ...
        <div className="px-3 py-2">
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-sm font-medium transition-all group"
          >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          ...
          <div className="space-y-1">
            {history.length > 0 ? (
              history.map((chat) => (
                <button key={chat.id} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/5 text-sm text-white/70 hover:text-white transition-all group text-left">
                  <MessageSquare size={16} className="shrink-0 text-white/30 group-hover:text-primary" />
                  <span className="truncate">{chat.title}</span>
                </button>
              ))
            ) : (
              <div className="px-3 py-4 text-xs text-white/20 italic">No recent chats</div>
            )}
          </div>
        </div>
